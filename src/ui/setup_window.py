...
class ProjectWizard(tk.Toplevel):
    ...
    def __init__(
        ...
        self.existing_project_dir: Optional[Path] = getattr(orchestrator, "current_project_dir", None) if mode == "edit" else None
...
Respond ONLY with valid JSON, no markdown."""
    ...
    def _build_final_config(self):
        ...
        base_desc = self.project_data['description'] or existing_meta.get('description') or (getattr(existing_cfg, 'description', '') if existing_cfg else '')
        ...
        if project_type == 'ml':
            ...
            problem_type = _safe_get(suggestions, 'problem_type', existing_meta.get('problem_type', 'time_series_forecasting'))
            ...
            batch_size = _safe_get_nested(suggestions, 'training_preset', 'batch_size', existing_meta.get('batch_size', 64))
            epochs = _safe_get_nested(suggestions, 'training_preset', 'epochs', existing_meta.get('epochs', 100))
            learning_rate = _safe_get_nested(suggestions, 'training_preset', 'learning_rate', existing_meta.get('learning_rate', 0.001))
            ...
            architecture = _safe_get(suggestions, 'architecture', architecture_existing or existing_meta.get('architecture', 'clean_architecture'))
            framework = _safe_get(suggestions, 'framework', framework_existing or existing_meta.get('framework', 'FastAPI'))
            database = _safe_get(suggestions, 'database', database_existing or existing_meta.get('database', 'PostgreSQL'))
    ...
    def _load_from_existing_project(self) -> None:
        ...
        try:
            is_ml = (getattr(cfg, 'project_type', '') == 'ml_competition') or (meta.get('project_type') == 'ml_competition')
        except Exception:
            is_ml = False
...
class RalphUI(tk.Tk):
    ...
    def _edit_current_project(self):
        ...
        if not getattr(self.orchestrator, 'current_config', None) or not getattr(self.orchestrator, 'current_project_dir', None):
...
📝 3-step wizard with Phase 2B PRD backlog expansion"""
    ...
    def _create_task_refinement_tab(self):
        ...
        ttk.Label(container, text="Refine your project's PRD/backlog or edit project details", font=('Arial', 11)).pack(pady=5)
        ...
        ttk.Button(btn_frame, text="✏️ Edit Project (3-Step Wizard)", command=self._edit_current_project).pack(side='left', padx=5)
